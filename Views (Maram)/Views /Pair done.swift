...
struct Pairdone: View {
    ...
    // Function to start the looping animation
    func startCheckmarkAnimation() {
        // Reset
        checkmarkProgress = 0.0
        
        // Animate drawing to 1
        withAnimation(.easeInOut(duration: 1.5)) {
            checkmarkProgress = 1.0
        }
        
        // Schedule next loop
        // The total cycle time (draw + pause)
        timer?.invalidate() // Stop any existing timer
        timer = Timer.scheduledTimer(withTimeInterval: 2.5, repeats: true) { _ in
            checkmarkProgress = 0.0 // Reset immediately (invisible)
            // Small delay to ensure it resets before drawing again
    ...
    var body: some View {
        ...
        // Start animation loop when view appears
        .onAppear {
        ...
        // Restart loop if swiped back to page 0
        .onChange(of: selectedPage) { newPage in
            if newPage == 0 {
                startCheckmarkAnimation()
            } else {
                timer?.invalidate() // Stop loop if we move away to save resources
            }
        }
    }
}
